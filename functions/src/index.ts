...
export const generateDoseInstances = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const schedulesByItemId = new Map<string, string[]>();
        schedulesSnap.docs.forEach(s => {
            const d = s.data();
            if (d.itemId && Array.isArray(d.times) && d.times.length > 0) {
                schedulesByItemId.set(d.itemId, d.times as string[]);
            }
        });
        ...
        for (const med of medications) {
            ...
            const schedule = (med.schedule || med.times || schedulesByItemId.get(med.id) || []) as string[];
            ...
            for (let d = 0; d < days; d++) {
                ...
                for (const time of schedule) {
                    ...
                    if (med.profileId) doseData.profileId = med.profileId;
...
export const generateMonthlyReport = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (dosesSnap.empty) return { message: 'Nenhum dado disponÃ­vel para este mÃªs', report: null };
