...
export default function AddMedicationWizard() {
  ...
  const processOCR = async (imageData: string) => {
    ...
    try {
      ...
      if (data?.name) {
        ...
        toast.success(t("toast.medication.dataExtracted"));
        setStep(1); // Go to first wizard step
      } else {
        toast.error(t("toast.medication.extractionFailed"));
        setImagePreview(null);
      }
    } catch (error) {
      console.error("OCR Error:", error);
      toast.error(t("toast.medication.imageError"));
      setImagePreview(null);
    } finally {
  ...
  const handleNext = () => {
    if (step === 1 && (!name || !dose)) {
      toast.error(t("toast.medication.fillNameDose"));
      return;
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (!user) throw new Error(t("toast.medical.notAuthenticated"));
      ...
      toast.success(t("toast.medication.addedSuccess"));
      navigate("/hoje");
    } catch (error) {
      console.error("Error saving item:", error);
      toast.error(t("toast.medication.saveError"));
    } finally {
      setSaving(false);
    }
  };
